...
namespace _231107C__ParkingCarManager
{
    ...
    // mysql이나 oracle 이랑 C# 연결하려면
    //nuget 패키지필요
    //하지만 mssql은 별도의 설치 없이 연결가능
    ...
    public class DBHelper
    {
        ...
        private const string TABLENAME = "parkingCarManager"; // 테이블 명
        ...
        // 주차 공간 검사, 전체 조회 등의 역활을 함
        
       // 디폴트 매개변수(일종의 오버로딩)
       // selectQuery(); 이렇게 호출시 ps에 -1이 자동할당됨
       // selectQuery("5"); 이렇게 호출시 ps에 "5"가 할당
        public static void selectQuery(string ps = "-1")
        {
            try
            {
                ...
                da = new SqlDataAdapter(cmd); // 쿼리문을 활용해 데이터 불러오는 것
                ds = new DataSet();
                da.Fill(ds, TABLENAME); // ds에 테이블을 채워넣음
                dt = ds.Tables[0]; // 만약 여러개의 테이블을 불러왔다면 그 중 첫번째 꺼 가지고옴
            }
            catch (Exception ex)
            {
            ...
            {
                conn.Close(); // DB 연결이 잘못되더라도 연결 끊어주는 건 꼭 해준다
            }
        }
        ...
        // 테이블에 있는 데이터 삭제하는 메소드 
        public static void deleteQuery(string ps) { }

        // 테이블에 있는 데이터 추가하는 메소드
